...
import vista_arc3.claude.controller as controller_module
from vista_arc3.claude.controller import (
    GameController,
    validate_tool_action,
)
from vista_arc3.claude.dispatcher import (
    GameToolDispatcher,
...
def test_fresh_session_reset_requires_a_bounded_retry_state() -> None:
...
def test_same_session_reset_needs_no_retry_state() -> None:
    ...
    unexpected = validate_tool_action(
    ...
    assert unexpected.error == "Unexpected tool argument."
...
def test_runtime_recovery_snapshots_state_without_game_action(tmp_path: Path) -> None:
    ...
    controller.complete_runtime_recovery(
        recovery,
        delivery="fresh_runtime_thread",
    )
    events = [
        json.loads(line)
        for line in (private / "compact_recovery.jsonl").read_text().splitlines()
    ]
    assert events[-1]["delivery"] == "fresh_runtime_thread"

    controller.complete_runtime_recovery(
        recovery,
        delivery="fresh_continuation_thread",
    )
    events = [
        json.loads(line)
        for line in (private / "compact_recovery.jsonl").read_text().splitlines()
    ]
    assert events[-1]["delivery"] == "fresh_continuation_thread"
...
def test_read_pixels_preserves_rendered_grid_colors(tmp_path: Path) -> None:
    initial = observation(value=15, frames=1)
    controller, _, _, frames = make_controller(
        tmp_path,
        render_scale=8,
        render_grid=True,
        initial_observation=initial,
    ...
    with Image.open(frames / "turn_000_frame_00.png") as supplied:
        assert supplied.getpixel((0, 0)) == (122, 64, 160)

    response = controller.handle(
        {
            "method": "read_pixels",
            "arguments": {
                "question": "What is the exact rendered corner color?",
                "views": [
                    {
                        "label": "corner",
                        "turn": 0,
    ...
    assert response["metadata"]["palette"] == {"0": "#7A40A0"}
    assert response["metadata"]["views"][0]["samples"] == ["0"]
...
def test_history_exposes_only_public_actions_and_environment_results(
    ...
    assert reset["metadata"]["visual"]["turn"] == 2
    controller.handle(
        {
            "method": "play",
            "request_id": 3,
...
def test_invalid_retry_limit_requests_recovery_without_ending_game(
    tmp_path: Path,
) -> None:
    ...
    generic_execution = execution_from_response(second)
    assert generic_execution.success is False
    assert generic_execution.interrupt_after is False

    execution = execution_from_response(
        second,
        interrupt_on_invalid_action_limit=True,
    )
    assert execution.success is False
    assert execution.interrupt_after is True
    assert execution.boundary_reason == "invalid_action_limit"
...
def test_game_over_exposes_only_reset_and_can_continue(tmp_path: Path) -> None:
    ...
    assert "retry_boundary" not in reset["metadata"]
    assert controller.retry_boundary_pending is False
    assert controller.step_index == 2


def test_same_session_reset_returns_visual_without_interrupting_play(
    tmp_path: Path,
) -> None:
    retry_marker = tmp_path / "private" / "retry_boundary.pending"
    ...
    working.write_text("Agent-owned scratchpad.\n", encoding="utf-8")
    reset = controller.handle(
        {
            "method": "play",
            "request_id": 2,
    ...
    execution = execution_from_response(reset)
    ...
    assert "retry_boundary" not in reset["metadata"]
    assert reset["metadata"]["visual"]["turn"] == 2
    assert "image" in reset
    assert execution.success is True
    assert execution.interrupt_after is False
    assert len(execution.images) == 1
    assert controller.retry_boundary_pending is False
    assert not retry_marker.exists()
    assert working.read_text(encoding="utf-8") == "Agent-owned scratchpad.\n"
    assert continued["metadata"]["action_applied"] is True
    assert [call["method"] for call in env.calls] == ["step", "reset", "step"]


def test_fresh_session_reset_preserves_working_and_blocks_until_recovered(
    tmp_path: Path,
) -> None:
    working = tmp_path / "visible" / "WORKING.md"
    guide = tmp_path / "visible" / "GUIDE.md"
    ...
    response = controller.handle(
        {
            ...
            "arguments": {
                "action": "RESET",
                "retry_state": "This is ignored because the game is terminal.",
            },
        }
    )
...
def test_level_boundary_archives_and_clears_working_without_blocking_play(
    tmp_path: Path,
) -> None:
    working = tmp_path / "visible" / "WORKING.md"
    guide = tmp_path / "visible" / "GUIDE.md"
    ...
    saved = dispatcher.execute(
        "write_working",
        {"content": "Temporary current-level state."},
        "save-working",
    )
    ...
    assert "memory_handoff" not in response["metadata"]
    ...
    assert archive_metadata["previous_progress"] == {"completed": 0, "total": 3}
    assert archive_metadata["current_progress"] == {"completed": 1, "total": 3}
    assert archive_metadata["saved_at"]["turn"] == 0
    recovery = controller.begin_runtime_recovery()
    ...
    current_working = dispatcher.execute("read_working", {}, "read-current-working")
...
def test_level_boundary_clears_empty_working_without_archive(tmp_path: Path) -> None:
    working = tmp_path / "visible" / "WORKING.md"
    guide = tmp_path / "visible" / "GUIDE.md"
    working.parent.mkdir(parents=True)
    working.write_text(" \n", encoding="utf-8")
    controller, _, private, _ = make_controller(
        tmp_path,
        initial_observation=observation(levels_completed=0, win_levels=3),
        next_observations=[observation(1, levels_completed=1, win_levels=3)],
        working_path=working,
        guide_path=guide,
    )
    response = controller.handle(
    ...
    assert not working.exists()
    assert not (private / "working_memory_provenance.json").exists()
    assert not (private / "working_memory_archive").exists()
...
def test_compact_recovery_moves_checkpoint_into_fresh_thread(tmp_path: Path) -> None:
    ...
    assert env.calls == []
    recovery_record = json.loads((private / "compact_recovery.jsonl").read_text())
    assert recovery_record["delivery"] == "fresh_thread"
    assert recovery_record["action_applied"] is False


def test_claude_checkpoint_is_committed_by_write_working(tmp_path: Path) -> None:
    private = tmp_path / "private" / "claude"
    private.mkdir(parents=True)
    checkpoint = private / "compact_checkpoint.requested"
    ready = private / "compact_checkpoint.ready"
    restore = private / "compact_restore.pending"
    snapshot = private / "compact_guide.snapshot"
    checkpoint.touch()
    guide = tmp_path / "visible" / "GUIDE.md"
    guide.parent.mkdir(parents=True)
    guide.write_text("Old durable model.\n", encoding="utf-8")
    working = guide.with_name("WORKING.md")
    working.write_text("Existing scratchpad.\n", encoding="utf-8")
    controller, env, _, _ = make_controller(
        tmp_path,
    ...
    blocked = dispatcher.execute(
        "play",
        {"action": "ACTION1"},
        "play-during-checkpoint",
    )
    blocked_inspect = dispatcher.execute(
        "inspect",
        {},
        "inspect-during-checkpoint",
    )
    blocked_history = dispatcher.execute(
        "history",
        {},
        "history-during-checkpoint",
    )
    blocked_read_guide = dispatcher.execute(
        "read_guide",
        {},
        "read-guide-during-checkpoint",
    )
    read_working = dispatcher.execute(
        "read_working",
        {},
        "read-working-during-checkpoint",
    )
    blocked_write_guide = dispatcher.execute(
        "write_guide",
        {"content": "Updated durable model."},
        "guide-during-checkpoint",
    )
    ready_after_blocked_guide = ready.exists()
    working_result = dispatcher.execute(
        "write_working",
        {"content": "Continue from the current state."},
        "working-during-checkpoint",
    )
    ...
    assert blocked_inspect.success is False
    assert blocked_inspect.text == blocked.text
    assert blocked_history.success is False
    assert blocked_history.text == blocked.text
    assert blocked_read_guide.success is False
    assert blocked_read_guide.text == blocked.text
    assert read_working.success is True
    assert read_working.text == "Existing scratchpad.\n"
    assert blocked_write_guide.success is False
    assert blocked_write_guide.text == blocked.text
    assert ready_after_blocked_guide is False
    assert working_result.success is True
    assert working_result.interrupt_after is True
    assert working_result.boundary_reason == "compact_checkpoint_saved"
    assert ready.is_file()
    assert guide.read_text(encoding="utf-8") == "Old durable model.\n"
    assert working.read_text(encoding="utf-8") == (
        "Continue from the current state.\n"
    )
    assert env.calls == []


def test_compact_checkpoint_request_does_not_apply_an_action(tmp_path: Path) -> None:
    marker = tmp_path / "private" / "codex_home" / "compact_checkpoint.requested"
    marker.parent.mkdir(parents=True)
    ready = marker.with_name("compact_checkpoint.ready")
    restore = marker.with_name("compact_restore.pending")
    snapshot = marker.with_name("compact_guide.snapshot")
    guide = tmp_path / "visible" / "GUIDE.md"
    guide.parent.mkdir(parents=True)
    guide.write_text("Durable model\n", encoding="utf-8")
    working = guide.with_name("WORKING.md")
    working.write_text("Current state\n", encoding="utf-8")
    controller, env, private, _ = make_controller(
        tmp_path,
        compact_restore_marker=restore,
        compact_guide_snapshot=snapshot,
        compact_checkpoint_marker=marker,
        compact_checkpoint_ready=ready,
        working_path=working,
        guide_path=guide,
    )

    controller.request_compact_checkpoint(reason="provider_image_limit")

    assert marker.is_file()
    assert controller.step_index == 0
    assert env.calls == []
    record = json.loads((private / "compact_recovery.jsonl").read_text())
    assert record == {
        "step": 0,
        "action_applied": False,
        "delivery": "checkpoint_requested",
        "reason": "provider_image_limit",
    }
...
def test_compact_recovery_failure_fails_closed(tmp_path: Path) -> None:
    marker = tmp_path / "private" / "codex_home" / "compact_restore.pending"
    marker.parent.mkdir(parents=True)
    snapshot = marker.with_name("compact_guide.snapshot")
    checkpoint = marker.with_name("compact_checkpoint.requested")
